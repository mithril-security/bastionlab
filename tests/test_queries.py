...
class TestingConnection(unittest.TestCase):
    ...
    def testingdf(self):
        ...
        client = connection.client
        policy = Policy(
            safe_zone=Aggregation(min_agg_size=1), unsafe_handling=Log(), savable=False, convertable=True
        )
    ...
    def testingquery2(self):
        ...
        connection = Connection("localhost", 50056)
        client = connection.client
        policy = Policy(safe_zone=Aggregation(1), unsafe_handling=Log(), savable=False, convertable=True)
        rdf = client.polars.send_df(df, policy)
        per_sex_rates = (
