...
def delegate(target_cls: Callable, target_attr: str, f_names: List[str], wrap: bool = False, wrap_fn: Optional[Callable] = None) -> Callable[[Callable], Callable]:
...
class RemoteLazyFrame:
    ...
    def composite_plan(self: LDF) -> str:
        segments = ",".join([seg.serialize() for seg in [*self._meta._prev_segments, PolarsPlanSegment(self._inner)]])
        return f'[{segments}]'
    ...
    def join(
        ...
        res = self._inner.join(other._inner, left_on, right_on, on, how, suffix, allow_parallel, force_parallel)
...
class FetchableLazyFrame(RemoteLazyFrame):
    ...
    def _from_reference(client: Client, ref: ReferenceResponse) -> LDF:
        ...
        df = pl.DataFrame([pl.Series(k, dtype=getattr(pl, v)()) for k, v in header.items()])
        return FetchableLazyFrame(_identifier=ref.identifier, _inner=df.lazy(), _meta=Metadata(client, [EntryPointPlanSegment(ref.identifier)]))
