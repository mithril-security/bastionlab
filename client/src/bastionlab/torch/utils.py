...
def serialize_model(
    model: Module, name: str, description: str, chunk_size: int = 100_000_000, progress: bool = False,
) -> Iterator[Chunk]:
    ...
    return data_chunks_generator(
        stream_artifacts(iter([ts]), chunk_size, torch.jit.save), name=name, description=description, meta=b"", progress=progress,
    )
