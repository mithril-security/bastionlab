...
def handle_download(path_str: str, url: str, name: str, error_msg: str):
    if path.exists(path_str) is False:
        ...
        try:
            ...
            raise NotFoundError("{}. Exact error code: {}".format(error_msg, e.code)) from None
    else:
        print("{} already installed".format(name))

def tls_certificates():
    if path.exists("./bin/tls") is False:
        os.system("mkdir -p bin/tls && openssl req -newkey rsa:2048 -nodes -keyout bin/tls/host_server.key -x509 -days 365 -out bin/tls/host_server.pem -subj ""/C=FR/CN=bastionlab-server""")
...
def start() -> Popen:
    ...
    bastion_url = "https://github.com/mithril-security/bastionlab/releases/download/v{}/bastionlab-{}-linux.zip".format(app_version, app_version)
    ...
    handle_download(bastionlab_path, bastion_url, "BastionLab server (version {})".format(app_version), "The release might not be available yet for the current version")
    handle_download(libtorch_path, libtorch_url, "Libtorch (version 1.12.1)", "Unable to download Libtorch")
    tls_certificates()
