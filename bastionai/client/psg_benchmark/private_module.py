...
class PrivacyEngine:
    ...
    def initialize_grad_sample_store(self, module: Module, max_batch_len: int):
        for _, param in trainable_parameters(module):
            ...
            module.grad_sample_store[param] = Parameter(
    ...
    def make_private(self, module: Module, max_batch_len: int) -> None:
        ...
        for _, m in trainable_modules(module):
            if (
                not type(m) in self.grad_sample_fn_table
                and not type(m) in PrivacyEngine.GLOBAL_SAMPLE_FN_TABLE
            ):
                continue
    ...
    def sample_backprop_hook(
        self,
    ) -> Callable[[Module, Tuple[Tensor, ...], Tuple[Tensor, ...]], None]:
        ...
        def inner(module, grad_input, grad_output):
            ...
            for param, grad_sample in grad_sample_fn(
                module, module.input.pop(), grad_output
            ).items():
                if param in module.grad_sample_store:
                    module.grad_sample_store[param][
                        : grad_sample.shape[0]
                    ] += grad_sample
